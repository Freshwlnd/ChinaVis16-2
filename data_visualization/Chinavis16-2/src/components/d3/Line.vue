  ...
  export default {
    ...
    methods: {
      init() {
        ...
        let width = this.width ? this.width : 2000;
        let height = this.height ? this.height : 1500;
        let padding = {
          left: 80,
          right: 50,
          top: 50,
        ...
        let xAxis = d3.axisBottom(xScale).tickFormat(d => d.getFullYear() )
        ...
        svg.append('g')
          .attr('transform', 'translate(' + 85 + ',' + padding.top + ')')
          .call(yAxis)
          .selectAll('text')
        ...
        this.dataset.forEach((v, vi) => {
          ...
          svg.append('text')
            .attr('dx', tp_x)
            .attr('dy', tp_y)
            .attr('transform', 'translate(-60,' + padding.top + ')')
            .text(v.name)
            .style("fill", colorZ(vi))
            .style('font-size', '40px')
        })
